...
export default function AddProduct() {
  ...
  return (
    <div className="add-product">
      ...
      <button className="rounded-full bg-yellow-300 p-4" onClick={() => dispatch(2)}>Multiply by 2</button>
        <input type="text" value={description} onChange={(e) => setDescription(e.target.value)} name="description" id="" placeholder="Write down product description" className="p-3" />
      </div>
      ...
      {!!imgSrc && (
        <ReactCrop crop={crop} onChange={(_, percentCrop) => setCrop(percentCrop)} onComplete={(c) => setCompletedCrop(c)} aspect={aspect}>
          <img ref={imgRef} alt="Crop me" src={imgSrc} style={{ transform: `scale(${scale}) rotate(${rotate}deg)` }} onLoad={onImageLoad} width={400} height={400} />
        </ReactCrop>
      )}
      {!!completedCrop && (
        <>
